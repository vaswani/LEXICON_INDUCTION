classdef GMCCA
    ...
    methods(Static)
        ...
        function agg_pi=find_matching(options, data)
            ...
            for t=1:options.T,
                inv_agg_pi= Util.inverse_perm(agg_pi);
                ...
                if isfield(data, 'true_pi')   % compute distance to true permutation.
                    hamming(t) = Util.hamming(Util.inverse_perm(agg_pi), data.true_pi); 
        ...
        function run(maxN)
            ...
            data.GX     = GMCCA.processGraph(source.G(pi.X,pi.X));
            
            data.Y      = target.features(pi.Y,:);
            data.Y(:,1) = log10(data.Y(:,1));
            data.wordsY = {target.words{pi.Y}};
            data.GY     = GMCCA.processGraph(target.G(pi.Y,pi.Y));
            
            data.source = source;
            data.target = target;
            
            agg_pi=GMCCA.find_matching(options, data);
