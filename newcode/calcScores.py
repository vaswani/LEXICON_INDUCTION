...
#the dictionary is contains the list of all matching obtained from the gold lexicon. dictionary has format key, key of values
...
#filtered dictionary is a dictioary with the key as a source word and the value is another dictionary with the key as the target words
#matches is a list of lists. Each row in the list has th format [source_word,matched_target_word,score]. The items are sorted according to score
def computePrecisionRecall(filtered_dictionary,matches,num_matches,dictionary_size,beta):
        ...
        if match[0] in filtered_dictionary:
            if match[1] in filtered_dictionary[match[0]] :
                ...
                if recall >= recall_cutoffs[recall_cutoff_index]: 
                    ...
                    print 'precision at recall ',recall_cutoffs[recall_cutoff_index],' is ',repr(precision),' and the F1 is ',repr(F1)
